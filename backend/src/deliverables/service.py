...
def _select_with_names():
    # Una sola query resuelve course_name y category_name (JOIN) para que el
    # front no tenga que pedir la materia/categoría por separado.
    return (
...
def _validate_refs(db: Session, course_id: int, category_id: int) -> None:
...
def get_deliverable(db: Session, id: int) -> dict:
    stmt = _select_with_names().where(models.Deliverable.id == id)
...
def create_deliverable(db: Session, deliverable_in: schemas.DeliverableCreate) -> dict:
    _validate_refs(db, deliverable_in.course_id, deliverable_in.category_id)
    deliverable = models.Deliverable(**deliverable_in.model_dump())
    db.add(deliverable)
    db.commit()
    db.refresh(deliverable)
    return get_deliverable(db, deliverable.id)


def update_deliverable(
    db: Session, id: int, deliverable_in: schemas.DeliverableUpdate
) -> dict:
    ...
    db.commit()
    return get_deliverable(db, id)


def delete_deliverable(db: Session, id: int) -> None:
    deliverable = db.get(models.Deliverable, id)
    if deliverable is None:
        raise NotFoundError(f"Entregable con id {id} no encontrado")
